...
local TABLE_KEY = 91
assert(param:add_table(TABLE_KEY, "TW_", 21), "could not add TW_ parameter table")
...
assert(param:add_param(TABLE_KEY, 21, "ACCEL_MIN", 45), "could not add TW_ACCEL_MIN")

local P = {}
for _, name in ipairs({
  "ENABLE", "LOG_ONLY", "FOLD_CH", "PWM_FW", "PWM_Q", "RATE_UP", "RATE_DN",
  "TIMEOUT", "OUT_GAIN", "THR", "ROLL", "PITCH", "YAW", "GUARD", "SAFE_MIN",
  "BLEND_AS", "FW_AS", "ATT_DANG", "DESC_DANG", "SAT_PWM", "ACCEL_MIN"
}) do
...
local function evaluate_transition(theta, target, airspeed, roll_deg, pitch_deg, climb_rate, saturation)
  ...
  if target < safe_min and target >= accel_min and airspeed < P.BLEND_AS:get() then
    risk = RISK.WARN
  end
  if target < accel_min and airspeed < P.FW_AS:get() then
    risk = RISK.DANGER
  end
  ...
  if risk == RISK.DANGER and target < accel_min then
    action = ACTION.HOLD_THETA
    allowed = accel_min
  end
  if risk == RISK.DANGER and target < safe_min and (
      math.abs(roll_deg) > P.ATT_DANG:get() or
      math.abs(pitch_deg) > P.ATT_DANG:get() or
      climb_rate < P.DESC_DANG:get() or
      saturation > 0.5
    ) then
    action = ACTION.HOLD_THETA
    allowed = safe_min
